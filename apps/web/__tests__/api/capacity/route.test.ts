...
describe("GET /api/capacity", () => {
  ...
  it("should return capacity for specific session when requested", async () => {
    ...
    expect(data.data.sessions.morning.displayTime).toBe("09:00-12:00 น.");
  });
  ...
  it("should hide exact free count when business logic requires", async () => {
    ...
    expect(data.data.sessions.morning.advancedCount).toBe(20); // Advanced count should still be visible
  ...
  it("should show เต็มแล้ว message when session is full", async () => {
    ...
    expect(response.status).toBe(200);
    expect(data.data.sessions.morning.availabilityStatus).toBe("FULL");
...
describe("POST /api/capacity (capacity check)", () => {
  beforeEach(() => {
    jest.clearAllMocks();
